...
cron.schedule("0 0 * * *", async () => {
  ...
  try {
    ...
    for (const client of activeClients) {
      client.daysLeft -= 1;
        console.log(`🔄 Updating client: ${client.email}, Days Left: ${client}`);
      // Send reminder email if 5 or fewer days left
      if (client.daysLeft > 0 && client.daysLeft <= 5) {
         await sendEmail(
  client.email,
  `⏳ Your ${client.subscription} plan expires in ${client.daysLeft} days`,
  `
  ...
  `
      ...
      await sendEmail(
  client.email,
  `❌ Your ${client.subscription} plan has expired`,
  `
    <h3>Hi ${client.name},</h3>
